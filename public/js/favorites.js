...
$(document).ready(() => {
  ...
  // onclick event that will delete the favorite from the database
  const deleteBtnEl = document.querySelectorAll('.deleteBtn');
  deleteBtnEl.forEach((button) => {
    button.addEventListener('click', () => {
      const imgEl = button.previousElementSibling.children[0].getAttribute('src');
      const encoded = encodeURIComponent(imgEl);
      console.log(encoded);
      // eslint-disable-next-line
      const titleEl = button.parentElement.parentElement.children[2].children[0].innerHTML;
      // console.log(titleEl);
      fetch(`/api/saved/${encoded}`, {
        method: 'DELETE',
